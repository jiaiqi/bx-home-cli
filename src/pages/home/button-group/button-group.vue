  ...
  methods: {
    ...
    getButtons () {
      ...
      this.$fetch('select', 'srvhealth_page_item_buttons_select', req, 'health').then(res => {
        if (res.success) {
          this.buttons = res.data.filter(item => item.display !== '否' && item.display !== '隐藏')
        }
      })
    },
    async getGroupList () {
    ...
    toPages (e) {
      ...
      let url = '';
      if (e && (!this.bindUserInfo || !this.bindUserInfo.store_user_no) && e.type !== 'health-manager' && e
        .navType) {
        uni.showModal({
          title: '提示',
      ...
      if (navType === 'miniProgram') {
        ...
        if ([ 'navigateTo', 'redirectoTo', 'switchTab', 'reLaunch' ].includes(navType)) {
          uni[ navType ]({
            url: url,
            fail (err) {
              uni.switchTab({
    ...
    async toGroup (e) {
      ...
      if (data && data.pg_no && data.gc_no) {
        // 跳到聊天页面  
        uni.navigateTo({
          url: `/publicPages/chat/chat?type=群组圈子&storeNo=${this.storeNo}&groupNo=${data.gc_no}&pg_no=${data.pg_no}&group_role=${data.group_role || '用户'}`
        });
      } else {
        // 跳到圈子信息页面
        uni.navigateTo({
