  ...
  <view class="page-wrap">
    ...
    <view class="manager-view">
      ...
      <view class="manager-box" v-if="storeInfo.mgmt_button_type === '自动'">
        ...
        <view
          class="box-item"
          v-for="item in childTable"
          @click="toChildService(item)"
        >
          ...
          <view class="box-item-content">
            <view
              class="label-icon shadow-blur bg-blue light"
              v-if="item.label"
            >
              <view
                class="text"
                v-for="text in item.label.slice(0, 4).split('')"
              >
                {{ text }}
              </view>
            </view>
        ...
        if (e.foreign_key.more_config && e.foreign_key.more_config.targetType) {
          ...
          uni.navigateTo({
            url: '/publicPages/list/list?pageType=list&serviceName=' +
              ...
              '&viewTemp=' +
              JSON.stringify(viewTemp) + '&foreign_key=' + JSON.stringify(foreign_key) + '&main_data=' + JSON.stringify(this.storeInfo)
          });
        }
      } else {
        uni.navigateTo({
