<template>
  <div class="device-data">
    <u-tabs :list="userList" :is-scroll="false" :current="currentTab" @change="changeTab"></u-tabs>

    <view class="last-data-box" v-if="isArray(deviceData) && deviceData.length > 0">
      <view class="record-title text-bold">最新数据</view>
      <view class="record-data">
        <view class="last-data" v-if="isArray(deviceData) && deviceData.length > 0">
          <text class="digital">
          ...
          <text class="unit" v-if="deviceInfo.record_table === 'bxiot_ap_user_bp_data'">毫米汞柱</text>
          <text class="unit" v-if="deviceInfo.record_table === 'bxiot_ap_user_bs_data'">毫摩尔每升</text>
        </view>
        ...
        <view class="chart-content" v-if="isArray(deviceData) && deviceData.length > 0">
          <uniEcCharts v-if="deviceInfo.record_table === 'bxiot_ap_user_bp_data'" class="uni-ec-charts"
            id="uni-ec-charts" :ec="BPChartOption"></uniEcCharts>
          <uniEcCharts v-if="deviceInfo.record_table === 'bxiot_ap_user_bs_data'" class="uni-ec-charts"
            id="uni-ec-charts" :ec="GluChartOption"></uniEcCharts>
        </view>
    ...
    <view class="history-record" v-if="deviceData && deviceData.length > 0">
      ...
      <view class="list-box cu-list">
        <view class="cu-item list-item" @tap="updateItem(item)" v-for="(item, index) in deviceData" :key="index">
          ...
          <view class="content">
            ...
            <view class="unit">
              <text v-if="deviceInfo.record_table === 'bxiot_ap_user_bp_data'">mmHg</text>
              <text v-if="deviceInfo.record_table === 'bxiot_ap_user_bs_data'">mmol/L</text>
            </view>
    ...
    <uni-load-more class="load-more" :status="loadStatus" v-if="deviceData.length !== 0"></uni-load-more>
  </div>
</template>
      ...
      async getData(loadType = 'refresh') {
        ...
        if (this.deviceInfo.record_table) {
          ...
          let req = {
            ...
            "colNames": ["*"],
            "order": [{
              colName: "measure_time",
              orderType: "desc"
            }],
            "condition": [{
                "colName": "serial_number",
                "ruleType": "eq",
                "value": this.deviceInfo.serial_number
              },
              {
                "colName": this.deviceInfo.user_id_col || "user",
                "ruleType": "eq",
                value: this.currentUser.dev_user_index
              }
            ],
            "page": {
              "pageNo": this.pageNo,
