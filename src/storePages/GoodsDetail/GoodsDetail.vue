	...
	<view>
		...
		>
			<swiper-item v-for="(item, index) in swiperList" :key="item.url">
				<video :src="item.url" controls v-if="item.file_type === '视频' && current === index" :id="item.store_video_file" :poster="item.videoPoster" @click.stop=""></video>
				<image
					:src="item.url"
					:lazy-load="true"
					mode="aspectFill"
		...
		<view class="goods-info">
			...
			<view class="handler-bar margin-top-xs">
				...
				<view class="number-box" v-if="inCartGoodsInfo && inCartGoodsInfo.goods_amount">
					...
					<view class="goods-amount flex-1 text-center" style="width: 50px;text-align: center;font-size: 16px;">{{ inCartGoodsInfo.goods_amount || '' }}</view>
		...
		<view class="detail">
			...
			<view class="image-box">
				<image
					...
					v-for="item in goodsDetailImage"
					@click="toPreviewImage(item.url)"
				></image>
			</view>
		</view>
...
export default {
	...
	methods: {
		...
		payOrder(e) {
			...
			let url = `/storePages/payOrder/payOrder?store_no=${goodsInfo.store_no}&goods_info=${encodeURIComponent(JSON.stringify(goodsInfo))}`;
