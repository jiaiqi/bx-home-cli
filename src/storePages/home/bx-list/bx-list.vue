	...
	computed: {
		...
		list_config() {
			...
			let obj = {
				...
				btn_cfg: {
					...
					show_public_btn: this.listConfig?.btn_cfg?.show_public_btn ?? config?.btn_cfg?.show_public_btn ?? null,
		...
		filterCols() {
			...
			if (typeof cols === 'object') {
				...
				arr = this.srvCols.filter(item => cols.includes(item.columns) && !['Image', 'String'].includes(item.col_type));
			} else {
				let defVal = {};
	...
	methods: {
		...
		handlerCustomizeButton(e) {
			...
			if (e.servcie_type === 'add') {
				...
				let url =
					`/publicPages/form/form?service=${e.service}&serviceName=${e.service_name}&type=${e.servcie_type}&fieldsCond=` + encodeURIComponent(JSON.stringify(fieldsCond));
				if (this.appName) {
		...
		clickAddButton(e) {
			if (e.button_type === 'add') {
				...
				let url = `/publicPages/form/form?type=add&serviceName=${e.service_name.replace('_select', '_add')}&fieldsCond=${JSON.stringify(fieldsCond)}`;

				url = `/publicPages/formPage/formPage?type=add&serviceName=${e.service_name.replace('_select', '_add')}&fieldsCond=${JSON.stringify(fieldsCond)}`;
		...
		async getListV2() {
			...
			let self = this;
			let colVs = await this.getServiceV2(this.serviceName, 'list', this.listType === 'proc' ? 'proclist' : 'list', app);
		...
		async clickFootBtn(data) {
			...
			if (buttonInfo.button_type === 'customize') {
				if (buttonInfo.operate_type === '删除') {
					...
					let app = this.appName || uni.getStorageSync('activeApp');
					let url = this.getServiceUrl(buttonInfo.application || app, buttonInfo.operate_service, buttonInfo.servcie_type);
					...
					let url = '/publicPages/list2/list2?pageType=list&serviceName=' + buttonInfo.service_name + '&cond=' + JSON.stringify(buttonInfo.operate_params.condition);
				...
				if (buttonInfo && buttonInfo.button_type === 'edit' && rowData.proc_instance_no) {
					uni.navigateTo({
						url: '/publicPages/procDetail/procDetail?proc_instance_no=' + rowData.proc_instance_no
					});
				}
			} else {
				...
				if (buttonInfo.button_type === 'detail' && this.customDetailUrl) {
					...
					if (targetUrl && targetUrl.indexOf('"value":""') !== -1) {
						...
						let url = `/publicPages/form/form?serviceName=${buttonInfo.service_name}&type=detail&fieldsCond=` + encodeURIComponent(JSON.stringify(fieldsCond));

						// if (this.list_config?.detailPage === 'childTableList' || this.moreConfig?.detailPage ===
						//   'childTableList') {
						url = `/publicPages/detail/detail?serviceName=${buttonInfo.service_name}&fieldsCond=${JSON.stringify(fieldsCond)}`;
						// }
						if (this.hideChildList) {
							url = `/publicPages/form/form?type=detail&serviceName=${buttonInfo.service_name}&fieldsCond=${JSON.stringify(fieldsCond)}`;
						}
						if (Array.isArray(condition) && condition.length > 0) {
							url += `&condition=${JSON.stringify(condition)}`;
						}
				...
				this.onButtonToUrl(data, this.appName).then(res => {
					...
					if (buttonInfo && buttonInfo.button_type === 'detail') {
						...
						let url = `/publicPages/formPage/formPage?type=detail&serviceName=${button.service_name}&fieldsCond=${JSON.stringify(fieldsCond)}`;
						// if (this.list_config?.detailPage === 'childTableList' || this.moreConfig?.detailPage ===
						//   'childTableList') {
						url = `/publicPages/detail/detail?serviceName=${button.service_name}&fieldsCond=${JSON.stringify(fieldsCond)}`;
						// }
						if (this.hideChildList) {
							url = `/publicPages/form/form?type=detail&serviceName=${button.service_name}&fieldsCond=${JSON.stringify(fieldsCond)}`;
						}
						if (this.appName) {
							url += `&appName=${this.appName}`;
						}
						...
						Object.keys(rowData).forEach(key => {
							if (
								!['id', 'modify_user_disp', 'modify_user', 'modify_time', 'create_user_disp', 'create_user', 'create_time', 'del_flag', '_buttons'].includes(key) &&
								rowData[key]
							) {
						...
						let url = `/publicPages/form/form?serviceName=${buttonInfo.service_name}&type=add&fieldsCond=${JSON.stringify(fieldsCond)}`;
						if (self.appName) {
						...
						uni.navigateTo({
							url: '/pages/public/proc/apply/apply?serviceName=' + buttonInfo.operate_service
						});
					}
				});
			}
