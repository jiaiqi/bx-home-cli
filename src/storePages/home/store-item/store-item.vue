<template>
	<view
		class="store-item"
		v-if="storeInfo && storeInfo.store_no && pageItem && isShow"
		:style="itemStyle"
		:class="{
	...
	>
		<view class="title" :style="titleStyle" v-if="pageItem && pageItem.show_label === '是' && pageItem.type !== '通用列表' && pageItem.type !== '疫苗列表'" @click="toMore">
		...
		<slide-list v-if="pageItem.type === '轮播图'" ref="swiperList" :storeInfo="storeInfo" :userInfo="userInfo" @setHomePage="setHomePage" :pageItem="pageItem"></slide-list>
		<store-info
			:storeInfo="storeInfo"
			:userInfo="userInfo"
			:bindUserInfo="bindUserInfo"
			@bindUser="bindStore"
			v-else-if="pageItem.type === '店铺信息' || pageItem.type === '店铺信息2'"
			:isBind="isBind"
			:pageItem="pageItem"
		...
		<goods-list
			...
			ref="goodsList"
		></goods-list>
		<vaccine-list :storeInfo="storeInfo" :pageItem="pageItem" v-else-if="pageItem.type === '疫苗列表'" ref="vaccineList"></vaccine-list>
		<staff-manage :storeNo="storeNo" :pageItem="pageItem" v-else-if="pageItem.type === '人员列表'" @toDoctorDetail="toDoctorDetail" ref="staffList"></staff-manage>
		<news-list
			:pageItem="pageItem"
		...
		></news-list>
		<notice-list v-else-if="pageItem.type === '通知横幅'" ref="noticeList" :storeNo="storeNo" :pageItem="pageItem"></notice-list>
		<relation-store v-else-if="pageItem && pageItem.type === '关联店铺'" ref="relationStore" :storeNo="storeNo" :pageItem="pageItem"></relation-store>
		<timeline-list
			:showProfile="false"
			...
			:limit="3"
			:showPublish="false"
			v-else-if="storeNo && pageItem && pageItem.type === '朋友圈' && userInfo && userInfo.userno"
		></timeline-list>
		<link-wifi :store_no="storeNo" v-else-if="storeNo && pageItem && pageItem.type === '连接WiFi' && room_no"></link-wifi>
...
export default {
	...
	methods: {
		...
		toOpenVip() {
			if (this.pageItem?.more_config?.open_url) {
				uni.navigateTo({
					url: this.pageItem?.more_config?.open_url
				});
			}
		},
		getQrcode(e) {
