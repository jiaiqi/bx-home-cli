<template>
	<view class="store-item" v-if="storeInfo && storeInfo.store_no && pageItem && isShow" :style="itemStyle" :class="{
		...
		<button-list :pageItem="pageItem" :userInfo="userInfo" :bindUserInfo="bindUserInfo" :storeInfo="storeInfo"
			...
			v-else-if="pageItem&&pageItem.type === '按钮组'"
			:button-list="pageItem.listdata" ref="buttonGroup">
		</button-list>
		...
		<timeline-list :showProfile="false" :profile="userInfo.profile_url" :beforeClick="beforeClick"
			:storeNo="storeNo" noMargin showMore :condition="timelinecondition" :limit="3" :showPublish="false"
		...
		<bx-list v-else-if="storeNo && pageItem && pageItem.type === '通用列表'" :beforeClick="beforeClick"
		...
		<view class="user-card" v-else-if="storeNo && pageItem && pageItem.type === '用户卡片'">
			<view class="left" v-if="!hasNotRegInfo">
				...
				<view class="nick-name">
					...
					<view class="text-orange" v-if="moreConfig&&moreConfig.showSubscribe&&!isAttention"
						@click="toOfficial(true)">
						点击关注公众号,及时获取消息通知!
					</view>
				</view>
			</view>
		...
		<vip-card :config="pageItem" v-else-if="storeNo && pageItem && pageItem.type === '会员卡片'"
			:beforeClick="beforeClick">
		</vip-card>

		<avatar-list :storeNo="storeNo" :page-item="pageItem"
			v-else-if="storeNo && pageItem && pageItem.type === '用户展示'" :beforeClick="beforeClick"></avatar-list>
		<!-- 公众号关注组件 -->
