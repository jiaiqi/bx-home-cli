			...
			<view class="image-bg" :style="{ 'background-image': backgroundImage }">
				<view class="store-info" v-if="setStoreInfo">
					...
					<view class="store-button">
						<button class="image-btn margin-right bg-white"
							style="border-radius: 50%;background-color: #fff;" @click.stop="showModal('showQrCode')">
							<image class="image" :src="require('../../static/qrcode1.png')" mode=""></image>
						</button>
						<button class="image-btn margin-right" @click.stop="toManage"
							v-if="isManager && showBtn.manage">
		...
		<view class="cu-modal bottom-modal" :class="{ show: modalName === 'showQrCode' }" @click="hideModal">
			<view class="cu-dialog " @click.stop="">
				...
				<view class="qrcodeCanvas-box" v-if="modalName === 'showQrCode' && qrCodeText">
					<uni-qrcode cid="qrcodeCanvas" style="width: 100px;height: 100px;" :text="qrCodeText"
						:size="codeSize" class="qrcode-canvas" foregroundColor="#333" makeOnLoad
						@makeComplete="qrcodeCanvasComplete" ref="qrcodeCanvas">
					</uni-qrcode>
				</view>
			</view>
		</view>
		...
		watch: {
			...
			storeInfo: {
				...
				handler(newValue, oldValue) {
					...
					this.getFilePath(this.setStoreInfo.logo).then(res => {
