		...
		<view class="cu-modal bottom-modal" @click="hideModal" :class="{ show: showHomePageSelector }">
			<view class="cu-dialog" @tap.stop>
				<view class="store-list" v-if="storeList.length > 0">
					<view class="store-item" v-for="item in storeList" :key="item.id" @click="switchStore(item)">{{ item.name || '' }}</view>
				</view>
			</view>
		</view>
			...
			<view class="cu-dialog" @tap.stop>
				<view class="store-user-box" v-if="bindUserInfo">
					<view class="profile-box">
						...
						<view class="user-name">{{ bindUserInfo.person_name || bindUserInfo.nick_name || '' }}</view>
						<view
							class="account-status cu-tag round sm light"
							:class="{
					...
					<view class="exit-store-user" v-if="bindUserInfo && bindUserInfo.store_user_no">
						...
						<button class="cu-btn bg-blue margin-right" :disabled="push_msg_set == bindUserInfo.push_msg_set && member_status !== '退出'" @click="savePushSet">
							保存设置
						</button>
...
export default {
	...
	methods: {
		...
		getConfig(pageItem) {
			if (pageItem && pageItem.type) {
				...
				switch (type) {
					case '店铺信息':
					case '店铺信息2':
						keys = ['show_consult', 'show_set_home', 'margin', 'type', 'swiper_image', 'show_set_home', 'image_origin', 'more_config'];
						break;
					case '按钮组':
						keys = [
							'show_subscribe',
		...
		async selectBindUser() {
			...
			if (Array.isArray(userList)) {
				...
				if (isBind && isBind.person_no) {
					...
					let invite_user_no = this.invite_user_no || this.inviterInfo?.invite_user_no || this.userInfo?.invite_user_no;
					if (invite_user_no) {
					...
					data: [
						{
							...
							person_name: this.userInfo && this.userInfo.name ? this.userInfo.name.replace(/\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDE4F]/g, '') : '',
							user_account: this.userInfo.userno,
							nick_name: this.userInfo && this.userInfo.nick_name ? this.userInfo.nick_name.replace(/\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDE4F]/g, '') : '',
							...
							invite_user_no: this.invite_user_no || this.userInfo?.invite_user_no || this.inviterInfo?.invite_user_no
						}
					]
				}
			];
				...
				success(result) {
					if (result.confirm) {
						self.$fetch('operate', 'srvhealth_person_info_update', req, 'health').then(res => {
							if (res.success && res.data.length > 0 && res.data[0].no === self.userInfo.no) {
								uni.showToast({
									title: '设置成功'
								});
		...
		async getPageDefine(home_page_no) {
			...
			const req = { serviceName: service, colNames: ['*'], condition: [{ colName: 'pt_no', value: home_page_no, ruleType: 'eq' }] };
			let res = await this.$http.post(url, req);
			if (Array.isArray(res.data.data) && res.data.data.length > 0) {
		...
		async getPageComponent(home_page_no) {
			...
			let req = {
				...
				colNames: ['*'],
				condition: [{ colName: 'pd_no', ruleType: 'eq', value: home_page_no }, { colName: 'display', ruleType: 'eq', value: '是' }],
				page: { pageNo: 1, rownumber: 99 },
				order: []
			};
			let res = await this.$http.post(url, req);
			this.pageItemList = []
		...
		async getTabbar(home_page_no) {
			...
			let req = {
				...
				colNames: ['*'],
				order: [{ colName: 'nav_seq', orderType: 'asc' }],
				condition: [{ colName: 'owner_pd_no', ruleType: 'eq', value: home_page_no }],
	...
	onShareAppMessage() {
		let path = `pages/home/home?from=share&store_no=${this.storeNo}&invite_user_no=${this.userInfo.userno}&share_type=bindOrganization&doctor_no=${this.userInfo.no}`;
	...
	async onLoad(option) {
		...
		if ((option.share_type === 'bindOrganization' || option.share_type === 'bind') && option.store_no && option.invite_user_no && !this.authBoxDisplay) {
			// 绑定诊所
			// 查找店铺用户列表
			this.storeNo = option.store_no;
