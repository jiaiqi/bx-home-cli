<template>
  <view>
    <view class="professor-box">
      <view class="professor-item" @click="toDoctorDetail">
        <image
          class="img"
          :src="
            getImagePath(professorInfo.profile_url)
              ? getImagePath(professorInfo.profile_url)
        ...
        <view class="doc-info">
          <view class="top">
            <view class="doc-name">
              <text>
                {{
                  professorInfo.person_name
                    ? professorInfo.person_name
                    : professorInfo.nick_name || ""
                }}</text
              >
            </view>
          </view>
          <view class="center">
            <view class="depart-name"
              >科室：{{ professorInfo.deptName || "耳鼻喉门诊科" }}</view
            >
          </view>
          <view class="bottom">{{
            professorInfo.staff_introduction || "暂无介绍"
          }}</view>
        </view>
      </view>
    </view>
    ...
    <view class="schedule-view">
      <view class="cuIcon-titles text-blue titles"> 出诊表 </view>
      <view class="time-bar">
        ...
        <view class="schedule-list">
          <view class="schedule-item" v-for="item in scheduleList">
            <view
              ...
              :class="{ 'bg-blue': item && item.am && item.am.total }"
            >
              <text v-if="item && item.am && item.am.total">{{
                item.am.reserved || 0
              }}</text>
              <text v-if="item && item.am && item.am.total"
                >/{{ item.am.total }}</text
            ...
            <view class="schedule-item-detail"> </view>
          </view>
        </view>
      </view>
    </view>
