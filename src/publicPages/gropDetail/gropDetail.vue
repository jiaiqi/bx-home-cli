<template>
  <view>
    ...
    <view
      class="setting-item cu-list group-member"
      v-if="type === 'group-member'"
    >
      ...
      <view class="see-all" v-if="loadmoreMemeber !== 'noMore'">
        <uni-load-more
          :status="loadmoreMemeber"
          :contentText="contentText"
          @clickLoadMore="selectGroupMember"
        >
        </uni-load-more>
      </view>
    </view>
    <view class="setting-item group-util" v-if="type === 'group-util'">
      ...
      <view class="content-list">
        ...
        <view class="content-item" @click="toPages('food')">
          <view class="image"
            ><text class="cuIcon-similar text-green"></text
          ></view>
    ...
    <view
      class="setting-item cu-list group-util"
      v-if="type === 'group-detail' || type === 'join-detail'"
    >
      ...
      <view class="cu-dialog radius" @click.stop="">
        <view class="qr-code">
          ...
          <uni-qrcode
            cid="qrcodeCanvas"
            class="qr-code-container"
            :text="'https://wx2.100xsys.cn/joinGroup/' + storeNo"
            :size="qrCodeSize"
            foregroundColor="#333"
            makeOnLoad
            @makeComplete="makeComplete"
            v-else-if="showQrCode && storeNo"
          ></uni-qrcode>
          <image
            :src="groupQrCode"
            mode="aspectFit"
          ...
          ></image>
          <view class="action"
            ><button class="cu-btn line-green text-green" open-type="share">
              分享
            </button>
          </view>
        </view>
    ...
    <view class="join-button" v-if="!joined && userInfo && userInfo.no"
      ><button class="cu-btn bg-blue button" @click="joinGroup">
        加入圈子
      </button></view
    >
  </view>
