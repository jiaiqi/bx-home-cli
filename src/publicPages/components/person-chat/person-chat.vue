      ...
      ">
        ...
        " :src="currentChat.voice_url" controls />
			...
			<scroll-view @scrolltoupper="loadData" :scroll-y="true" :scroll-top="scrollTop"
				:scroll-into-view="chatTextBottom" :scroll-with-animation="true" class="msg-list" :class="{
          ...
          ">
            ...
            " class="person-chat-item-accept" @longpress.stop="longPress(item)">
                  ...
                  : item.msg_content
							...
							<view class="article-main">
								...
								<view class="article-icon">
									<image class="image" :src="getImagePath(item.attribute.icon_image)"
										mode="scaleToFill" v-if="item.attribute && item.attribute.icon_image" />
									<image v-else class="image" :src="'../../static/news.png'" mode="scaleToFill" />
								</view>
							</view>
						</view>
						<view class="person-chat-item-right goods-card"
							v-else-if="item.msg_content_type === '商品'&&item.attribute" @click="toGoods(item)">
							<view class="goods-image">
								<image class="goods-image" :src="getImagePath(item.attribute.goods_img,true)"
              ...
              " class="map-container" @tap="openMap(item)">
						...
						<view @tap.stop.prevent="openVideo(item)"
                ...
                " controls></video>
                    ...
                    " src="/static/img/word.png" mode=""></image>
									<image v-else-if="item.file_type && item.file_type === 'xlsx'"
										src="/static/img/excel.png" mode="">
									</image>
									<image v-else-if="item.file_type && item.file_type === 'pdf'"
										src="/static/img/pdf.png" mode="">
									</image>
									<image v-else-if="
                      item.file_type && ['ppt', 'pptx'].includes(item.file_type)
                    " src="/static/img/ppt.png" mode=""></image>
									<image v-else-if="item.file_type && item.file_type === 'file'"
										src="/static/img/file1.png" mode="">
									</image>
									<image v-else-if="
                    ...
                    " src="/static/img/music.png" mode=""></image>
							...
							<view class="documents-item img" v-else @click.stop="">
								<image :src="item.pic_url" @click.stop="toPreviewImage(item.pic_url)"
									mode="scaleToFill" />
								<!-- <image
			...
			<view class="person-chat-bot-top">
				<view class="person-chat-left">
					...
					<view class="send-text" v-else>
						<textarea class="send-value" :adjust-position="false" :show-confirm-bar="false"
							...
							@input="inputChange" maxlength="-1" @confirm="sendMessage" confirm-type="send" v-if="
              ...
              " />
						<input class="send-value" :adjust-position="false" :show-confirm-bar="false" v-model="chatText"
							:hold-keyboard="false" type="text" @blur="onBlur" @focus="onInput" @input="inputChange"
							maxlength="-1" @confirm="sendMessage" confirm-type="send" v-else />
					</view>
				</view>
				<view class="person-chat-rig" :class="{ 'is-feed': isFeed }">
					<view class="person-chat-rig-add-wrap">
						<view @click="openLink" v-if="!isFeed" class="person-chat-rig-add"><text
								class="cuIcon-roundadd"></text>
						</view>
						<text class="send" :class="'bx-bg-'+theme" @touchend.prevent="sendMessage"
							v-if="isFeed">发送</text>
					</view>
				</view>
			</view>
			...
			async initMessageList(type = null, dontEmit) {
				...
				if (dontEmit !== true) {
					if (Array.isArray(res.data.data) && res.data.data.length > 0) {
						this.$emit('load-msg-complete', res.data.data[0], res.data.page);
					} else {
						this.$emit('load-msg-complete', res.data.data, res.data.page);
					}
				}
