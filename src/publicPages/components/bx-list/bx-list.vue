<template>
	<view class="list-wrap">
		<view class="list-wrap">
			...
			<sPullScroll ref="pullScroll" :heightStyle="heightStyle" :pullDown="pullDown" :pullUp="loadData"
				:enablePullDown="enablePullDown" :enablePullUp="enablePullUp" :top="top" :fixed="fixed" :bottom="bottom"
				finishText="我是有底线的...">
				<view :class="{ 'grid-layout': onlyShowTitle }" v-if="listType === 'list'">
					<list-item class="list-item-wrap" :class="{ 'col-4': colnumber === 4 }" :detailList="detailList"
						v-for="item in listData" :key="item[rowKey]" :itemData="item" :labels="ShowLabelColumn"
...
<script>
	...
	import sPullScroll from '@/publicPages/components/s-pull-scroll';
	export default {
		name: 'List',
		components: {
			listItem,
		...
		computed: {
			...
			configViewTemp() {
				...
				} else if (this.moreConfig && this.moreConfig.appTempColMap) {
			...
			ShowLabelColumn() {
				...
				} else if (this.moreConfig && this.moreConfig.showLabelColumn) {
					return this.moreConfig.showLabelColumn
