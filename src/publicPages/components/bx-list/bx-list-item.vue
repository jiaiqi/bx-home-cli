<template>
  <view
    class="list-item-wrap bg-white"
    :class="{ 'grid-layout-item': layout === 'grid' }"
  >
    <view class="list-item flex" v-if="viewType === 'normal'">
      <view class="main-image" v-if="itemData[viewTemp['img']]">
        <image
          class="image"
          :src="getImagePath(itemData[viewTemp['img']])"
          @click.stop="listItemClick"
          mode="aspectFill"
        ></image>
      </view>
      ...
      <view
        class="content-box flex-twice"
        v-if="listType === 'proc' && pageType === 'proc'"
      >
        ...
        <view class="footer" v-if="rowButton.length > 0">
          <view class="footer-btn" v-if="showFootBtn">
            <text
              ...
              v-for="item in theRowbutton"
              :key="item.id"
              @click.stop="footBtnClick(item)"
            >
              {{ item.button_name }}
            </text>
          </view>
      ...
      <view class="content-box flex-twice" v-else>
        ...
        <view class="footer">
          <text
            class="foot-name"
            v-if="goodsData.footer || goodsData.footer === 0"
            @click.stop="listItemClick"
          >
