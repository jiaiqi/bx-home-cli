	...
	<view class="page-wrap" :class="['theme-'+theme]">
      ...
      ">
			<view class="util-item bg-white round" @click="toPages('groupAnnouncement')"
				v-if="(groupInfo && groupInfo.gc_no&&sessionType!=='专题咨询') || sessionType === '店铺机构全员'">
		...
		<!-- 付费咨询 -->
		<view class="pay-consult" v-if="sessionType==='专题咨询'&&vvipCard">
			<view class="left">
			...
			<view class="right">
				...
				<view class="right-item" @click="stopPaidConsult" v-else-if="payConsultInfo.status==='open'">
					<text class="cuIcon-roundcheck icon"></text>
					<text>结束</text>
				</view>
			</view>
		...
		<view class="chart-page">
			<person-chat ref="chatInstance" :payConsultStatus="payConsultStatus"
				...
				:sessionInfo="sessionInfo" page-type="session" @clickAvatar="clickAvatar"
				@load-msg-complete="loadMsgComplete" @msgSendSuccess="msgSendSuccess" :groupInfo="groupInfo"
				:rowInfo="rowInfo" :storeInfo="storeInfo" :sessionType="sessionType" :storeNo="storeNo"
				:topHeight="topHeight" :group-no="groupNo" :receiverInfo="receiverInfo" :banSend="banSend||stopConsult"
				:band-post="bandPost" v-if="session_no&&storeUserInfo" :storeUserInfo="vstoreUser"
				:queryOption="queryOption"></person-chat>
		</view>
	</view>
</template>
			...
			banSend() {
				...
				if (this.bandPost === '仅用户禁言') {
					if (this.storeUserInfo.user_role) {
						if (this.storeUserInfo.user_role.indexOf(',') === -1 && this.storeUserInfo.user_role.indexOf(
								'用户') !== -1) {
							return true
						}
					}
