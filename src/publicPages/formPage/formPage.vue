...
<script>
  ...
  import fomrMixin from '@/publicPages/mixins/form.mixin.js'
  ...
  export default {
    ...
    async onLoad(option) {
      ...
      if (['detail', 'update'].includes(option.type) && option.id) {
        this.condition = [{
          colName: 'id',
          ruleType: 'eq',
          value: option.id
        }]
        if (!this.fieldsCond || (Array.isArray(this.fieldsCond) && this.fieldsCond.length === 0)) {
          this.fieldsCond = [{
            column: 'id',
            ruleType: 'eq',
            value: option.id
