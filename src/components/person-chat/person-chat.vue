<template>
  ...
  >
        ...
        >
          ...
          >
            <view
              ...
              :class="{ show: item.showRecall === true }"
              @click.stop=""
            >
            ...
            <view
              class="sender-info"
              v-if="item.sender_name || item.sender_group_role || item.identity"
            >
              ...
              <text
                ...
                v-else-if="item.identity && item.identity !== '客户'"
                >{{ item.identity }}</text
              >
            ...
            <view
              class="person-chat-item-left"
              @tap.stop="clickAvatar(item)"
            ...
            >
              <image
                ...
                "
                mode="aspectFill"
                v-if="identity === '客户' && item.identity === '客服'"
              ></image>
              <image
                :src="getImagePath(item.sender_profile_url)"
                mode="aspectFit"
                v-else-if="identity === '客服' && identity !== item.identity"
              ></image>
              <image
                :src="
                  identity === '患者'
                ...
                "
                mode="aspectFit"
                v-else-if="!groupNo && identity && rowInfo && rowInfo.row_no"
              ></image>
              <image
                :src="
            ...
            >
              ...
              >
                ...
                <view class="documents-item-right">
                  ...
                  ></image>
                  <image
                    v-else-if="item.file_type && item.file_type === 'xlsx'"
    ...
    <view
      class="person-chat-bot"
      :class="{ showLayer: !isFeed && isSendLink, 'ban-send': banSend }"
    >
      <view v-if="banSend" class="band-tip"
        >您已被管理员禁言,请联系管理员解除禁言</view
      >
      <view class="person-chat-bot-top">
        <view class="person-chat-left">
          <text
            class="image-icon cuIcon-sound"
            @click="changeVoice('keyword')"
            v-if="currentVoiceType === 'voice'"
          ></text>
          <text
            class="image-icon cuIcon-keyboard"
        ...
        <view class="person-chat-rig" :class="{ 'is-feed': isFeed }">
          <view class="person-chat-rig-add-wrap">
            ...
            ></view>
            <text class="send" @touchend.prevent="sendMessage" v-if="isFeed"
              >发送</text
            >
          </view>
        </view>
      ...
      <view
        class="person-chat-bot-bot"
        :class="{ showLayer: isSendLink && !showKeyboard }"
      >
        <view
          @click="openMenuPoup('question')"
          class="person-chat-bot-bot-item"
        >
  ...
  .person-chat-top {
    ...
    .msg-list {
      ...
      &.showKeyboard {
        ...
        &.showLayer {
          height: calc(
            100vh - var(--keyboard-height) - var(--window-top) - 55px - 230px
          );
        }

        &.top-height {
          height: calc(
            100vh - var(--keyboard-height) - var(--window-top) - 42px - 55px
          );
