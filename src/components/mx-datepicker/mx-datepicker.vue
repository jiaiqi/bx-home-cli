          ...
          <block v-if="isMultiSelect">
            <view class="picker-display">
              ...
              <view v-if="isContainTime" class="picker-display-link" :hover-stay-time="100"
                hover-class="picker-display-link-active" :style="{color}" @click="onShowTimePicker('begin')">
                {{BeginTimeTitle}}
              </view>
            </view>
            <view class="picker-display">
              ...
              <view v-if="isContainTime" class="picker-display-link" :hover-stay-time="100"
                hover-class="picker-display-link-active" :style="{color}" @click="onShowTimePicker('end')">
                {{EndTimeTitle}}
              </view>
            </view>
          </block>
    ...
    <view v-if="showTimePicker" class="picker">
      <view class="picker-modal picker-time">
        ...
        <view class="picker-modal-footer">
          ...
          <view class="picker-modal-footer-btn">
            <view class="picker-btn" :hover-stay-time="100" hover-class="picker-btn-active" @click="onCancelTime">取消
            </view>
            <view class="picker-btn" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active"
      ...
      procCalendar(item) {
        ...
        if (this.checkeds.length == 2) {
          ...
          if (DateTools.isSameDay(this.checkeds[1], item.dateObj)) { //结束日期
            if (this.isMultiSelect && this.showTips) item.tips = item.bgStyle.type ? this.beginText + ' / ' + this
...
<style lang="scss" scoped>
  ...
  $calendar-height: 720upx;
  // $calendar-size: 630upx;
  $calendar-item-size: 90upx;
  $calendar-item-height: 120upx;
  $calendar-item-radius:10rpx;
  ...
  .picker {
    ...
    &-modal {
      ...
      &-body {
        ...
        height: $calendar-height !important;
        position: relative;
      }
    ...
    &-calendar {
      ...
      &-view {
        ...
        &-item {
          ...
          width: 90%;
          height: 90%;
          border-radius: $calendar-item-radius;
          display: flex;
          align-items: center;
