  ...
  <view class="imageUploadContainer">
    <view class="imageUploadList">
      <view class="imageItem" v-bind:key="index" v-for="(path, index) in imageListData">
        <image :src="path" :class="{ dragging: isDragging(index) }" @tap="previewImage" :data-index="index"
      ...
      ></view> -->
      <button class="imageUpload"  @tap="selectImage" @chooseavatar="chooseavatar"
        v-if="isShowAdd && eventType !== 'navTo'">
        <text class="cuIcon-camera"></text>
      </button>
      <view v-if="isShowAdd && eventType === 'navTo'" class="imageUpload" @tap="toPage"><text
          class="cuIcon-camera"></text></view>
    </view>
    <image v-if="showMoveImage" class="moveImage" :style="{ left: posMoveImageLeft, top: posMoveImageTop }"
...
<script>
  export default {
    ...
    props: ['value', 'enableDel', 'enableAdd', 'enableDrag', 'serverUrl', 'formData', 'header', 'limit', 'fileKeyName',
      'showUploadProgress', 'serverUrlDeleteImage', 'eventType', 'openType'
    ],
    data() {
      return {
        imageBasePos: {
