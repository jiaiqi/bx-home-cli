  ...
  <view class="datatime">
    <picker mode="multiSelector" :range="range" range-key="text" @change="change" @columnchange="columnchange"
      ...
      <view class="content" :class="{ placeholder: !dateStr }">
        <text>{{ dateStr ? dateStr :disabled?'当前字段不支持编辑': placeholder }}</text>
      </view>
    </picker>
  </view>
</template>
    ...
    methods: {
      ...
      columnchange(event) {
        ...
        if (columnIndex == 0 || columnIndex == 1 || columnIndex == 2) {
          ...
          // 更新时数据
          this.setHourData(this.range[0][this.value[0]].number, this.range[1][this.value[1]].number, this.range[2][this
            .value[2]
          ].number);
          // 更新 Hour Value
          let hourIndex = this.range[3].findIndex(hour => {
        ...
        // 更新分数据
        this.setMinuteData(this.range[0][this.value[0]].number, this.range[1][this.value[1]].number, this.range[2][this
          .value[2]
