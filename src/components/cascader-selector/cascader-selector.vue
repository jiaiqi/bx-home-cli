<template>
  <view class="cascader-wrap">
    <cascader :insert="insert" :hideBorder="hideBorder" :hidePath="hidePath" @tag-click="clickTag"
      :areaList="oldAreaList" :currentNo="current_no" @line-click="clickLine" @show-more="showMore"
      :showSelect="showSelect" :column="srvInfo.column||srvInfo.refed_col"
      :showCol="srvInfo.showCol||srvInfo.key_disp_col" :isShowMore="isShowMore" :lineDataDefault="lineDataDefault">
    </cascader>
    <view class="button-box" v-if="!hideButton">
      <button class="cu-btn bg-green" @click="resetData">重 置</button>
      <button class="cu-btn bg-blue"
        @click="emitSelectVal">{{ lineDataDefault[lineDataDefault.length - 1] ? '确 定' : '取 消' }}</button>
    </view>
  </view>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      async getAreaData(cond, ismore, defaultVal, index, lastListIndex) {
        ...
        const appName = srvInfo.srv_app || srvInfo.appNo || this.srvApp;
        const that = this;
        ...
        if (cond && !ismore) {
          ...
          condition = [{
            colName: srvInfo?.parent_col|| this.srvInfo?.parentCol || 'parent_no',
            ruleType: 'isnull'
          }];
        }
        let req = {
      ...
      clickTag(e) {
        ...
        if (e[this.srvInfo?.column || 'no']) {
          ...
          } else if (e[this.srvInfo?.parentCol || 'parent_no'] === this.outputData[this.srvInfo?.parentCol ||
              'parent_no']) {
            // 同级
            this.lineDataDefault.splice(this.lineDataDefault.findIndex(item => item[this.srvInfo?.parentCol ||
              'parent_no'] === e[this.srvInfo?.parentCol || 'parent_no']), 1);
          } else if (this.lineDataDefault.findIndex(item => item[this.srvInfo?.parentCol || 'parent_no'] === e[this
              .srvInfo?.parentCol || 'parent_no']) !== -1) {
            this.lineDataDefault.splice(this.lineDataDefault.findIndex(item => item[this.srvInfo?.parentCol ||
              'parent_no'] === e[this.srvInfo?.parentCol || 'parent_no']), this.lineDataDefault.length);
          }
          this.outputData = e;
      ...
      clickLine(e, index) {
        ...
        if (e && e[this.srvInfo?.column || 'no']) {
          if (e[this.srvInfo?.parentCol || 'parent_no'] === null) {
            ...
            // 同级
            this.lineDataDefault.splice(this.lineDataDefault.findIndex(item => item[this.srvInfo?.parentCol ||
              'parent_no'] === e[this.srvInfo?.parentCol || 'parent_no']) + 1, 1);
          } else if (this.lineDataDefault.findIndex(item => item[this.srvInfo?.parentCol || 'parent_no'] === e[this
              .srvInfo?.parentCol || 'parent_no']) !== -1) {
            // 父级
            this.lineDataDefault.splice(this.lineDataDefault.findIndex(item => item[this.srvInfo?.parentCol ||
              'parent_no'] === e[this.srvInfo?.parentCol || 'parent_no']) + 1, this.lineDataDefault.length);
          }
          this.outputData = e;
