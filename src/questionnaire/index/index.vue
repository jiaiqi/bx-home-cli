<template>
  ...
  >
    <view
      class="to-history"
      v-if="configCols && configCols.length > 0"
      @click="toHistory"
      >点击查看历史提交</view
    >
    ...
    >
      <button
        ...
        type=""
        @click="seeScore"
        v-if="!scoreInfo.score && scoreInfo.score !== 0"
      >
        查看分数
      </button>
    ...
    <view class="button-box" v-if="showNextBtn"
      ><button class="button cu-btn" @click="toNextPages">下一步</button>
    </view>
  </view>
</template>
                ...
                self.$http.post(url, req).then(res => {
                  if (res.data.state === 'SUCCESS') {
                    if (res.data.resultCode === 'SUCCESS') {
                      ...
                      } else if (self.target === 'queue' && self.params.fill_batch_no && self.queue_no && self.store_no) {
                        uni.redirectTo({ url: `/storePages/queue/queue?fill_batch_no=${self.params.fill_batch_no}&queue_no=${self.queue_no}&store_no=${self.store_no}` })
                      } else {
                        uni.showModal({
                          title: '提示',
