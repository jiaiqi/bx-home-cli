<template>
  <view class="card-wrap">
    <view class="card">
      <view class="user-info">
        <view class="profile-box">
          <image :src="avatarUrl" class="image" mode="aspectFit"></image>
        </view>
        <view class="name-box">
      ...
      <view class="qr-code" v-if="userInfo && userInfo.no">
        <uni-qrcode cid="qrcodeCanvas" :text="'https://wx2.100xsys.cn/mpwx/' + userInfo.no" :size="size"
          foregroundColor="#333" makeOnLoad @makeComplete="qrcodeCanvasComplete" :class="{
    ...
    <view class="button-box">
      ...
      <button type="primary" class="cu-btn bg-blue" data-type="seeDoctor" v-if="myHospital && myHospital.length > 1"
        @click="inviteSeeDoctor">
      ...
      <view class="cu-dialog">
        <view class="select-hospital">
          ...
          <bx-radio-group class="form-item-content_value radio-group" mode="button" v-model="hospitalNo">
            <bx-radio class="radio" color="#2979ff" v-for="item in myHospital" :name="item.store_no">
              {{ item._store_no_disp }}</bx-radio>
          </bx-radio-group>
        </view>
      </view>
    </view>
    ...
    onShareAppMessage(e) {
      ...
      if (e.target && e.target.dataset && e.target.dataset.type === 'seeDoctor') {
        ...
        if (this.userInfo && this.userInfo.no) {
          ...
          title = `${this.userInfo.name}邀请你体验【百想首页】小程序`;
        }
