<template>
	<view class="history-wrap">
		...
		<view class="history-chart" v-if="isAllPages">
			<uniEcCharts class="uni-ec-charts" id="uni-ec-canvas" v-if="currentChart === 'stepChart'" :ec="stepEcData">
			</uniEcCharts>
			<uniEcCharts class="uni-ec-charts" id="uni-ec-canvas" v-if="currentChart === 'canvasLineA'"
				:ec="weightEcData"></uniEcCharts>
			<uniEcCharts class="uni-ec-charts" id="uni-ec-canvas" v-if="currentChart === 'canvasLineB'" :ec="bpEcData">
			</uniEcCharts>
			<uniEcCharts class="uni-ec-charts" id="uni-ec-canvas" v-if="currentChart === 'canvasColumnD'"
				:ec="caloriesEcData"></uniEcCharts>
			<uniEcCharts class="uni-ec-charts" id="uni-ec-canvas" v-if="currentChart === 'canvasLineC'"
				:ec="sleepEcData"></uniEcCharts>
		</view>
		<view class="button-box" v-if="isAllPages">
			<button class="buttons" :class="{ active: currentChart === item.chartID }" size="mini"
				v-for="item in subList" :key="item.key" @click="showCanvas(item.key)">
				{{ item.name }}
			</button>
		</view>
		<view class="history-box" v-if="pageType">
			<view class="symptom-page" v-if="pageType === 'symptom'">
				<view class="symptom-item-list" v-if="symptomRecord.length > 0">
					<view class="symptom-item" v-for="item in symptomRecord" :key="item.id">
						<view class="name">{{
              item.symptoms_name ? item.symptoms_name : ""
            }}</view>
						<view class="describe">{{
              item.symptoms_remark ? item.symptoms_remark : ""
            }}</view>
						<view class="date">{{ item.create_time.slice(5, 16) }}</view>
					</view>
				</view>
				<u-empty text="无历史记录" mode="history" v-else></u-empty>
				<button class="cu-btn bg-cyan symptom-add-button shadow-blur" v-if="pageType === 'symptom'"
					@click="toPages('symptom')">
					添加
				</button>
			</view>
			<view class="history-content">
				...
				<view class="other-record" v-if="
            pageType === 'sleep' || pageType === 'bp' || pageType === 'weight'
          " :class="{
            ...
            'weight-record': pageType === 'weight',
					...
					<view class="record-data" v-if="pageType !== 'weight'">
						<view class="last-data" v-if="
                isArray(historyRecord) &&
              ...
              ">
                      ...
                      'text-yellow':
                        historyRecord[0].systolic_pressure >= 120 &&
                        historyRecord[0].systolic_pressure < 140,
                      ...
                      'text-yellow':
                        historyRecord[0].diastolic_pressure < 90 &&
                        historyRecord[0].diastolic_pressure >= 80,
                ...
                pageType === 'sleep'
              ">
							<text class="digital" v-if="historyRecord[0].sleep_time">{{
                historyRecord[0].sleep_time.slice(0, 5)
              }}</text>
						...
						<button class="nav-button" @click="toPages(pageType)" v-if="!this.customer_no">
							记录数据
						</button>
					</view>
					<view class="bmi-box" v-if="pageType === 'weight'">
						<view class="bmi-bar-box" v-if="bmi">
							<view class="last-data">
								<view class="text-gray create-time"
									v-if="isArray(historyRecord) && historyRecord.length > 0">
									{{ historyRecord[0].create_time }}
								</view>
					...
					<view class="data-chart" v-if="pageType === 'weight' || pageType === 'bp'">
						<uniEcCharts class="uni-ec-charts" id="uni-ec-charts" :ec="weightChartOption"
							v-if="pageType === 'weight'"></uniEcCharts>
						<uniEcCharts class="uni-ec-charts" id="uni-ec-charts" :ec="BPChartOption"
							v-if="pageType === 'bp'"></uniEcCharts>
					</view>
					<view class="history-record">
						...
						<view class="list-box cu-list" v-if="historyRecord && historyRecord.length > 0">
							<view class="cu-item list-item" :class="modalName == 'move-box-' + index ? 'move-cur' : ''"
								@touchstart="ListTouchStart" @touchmove="ListTouchMove" @touchend="ListTouchEnd"
								:data-target="'move-box-' + index" @tap="updateItem(item)"
								v-for="(item, index) in historyRecord" :key="index">
								<image src="../static/icon/xueya.png" mode="" class="icon" v-if="pageType === 'bp'">
								</image>
								<image src="../static/icon/sleep.png" mode="" class="icon" v-if="pageType === 'sleep'">
								</image>
								<image src="../static/icon/tizhong.png" mode="" class="icon"
									v-if="pageType === 'weight'"></image>
								<view class="content">
									<view class="item">
										<text class="digital" :class="{
                        'text-green': item.systolic_pressure < 120,
                        'text-yellow':
                          item.systolic_pressure >= 120 &&
                          item.systolic_pressure < 140,
                        'text-red': item.systolic_pressure >= 140,
                      }" v-if="pageType === 'bp' && item && item.systolic_pressure">
											{{
                        item.systolic_pressure
                          ? getFixedNum(item.systolic_pressure)
                          : "-"
                      }}
										</text>
										<text class="digital" v-if="pageType === 'weight'">{{
                      item.weight ? getFixedNum(item.weight) : "-"
                    }}</text>
										<text class="digital" v-if="pageType === 'sleep'">{{
                      item.sleep_time ? item.sleep_time.slice(0, 5) : "-"
                    }}</text>
									</view>
                        ...
                        'text-yellow':
                          item.diastolic_pressure < 90 &&
                          item.diastolic_pressure >= 80,
                        'text-red': item.diastolic_pressure >= 90,
                      }" class="digital bp">
											{{
                        item.diastolic_pressure
                          ? getFixedNum(item.diastolic_pressure)
                          : "-"
                      }}
										</text>
									</view>
		...
		<view class="cu-modal bottom-modal" :class="{ show: showSymptom }" @click="showSymptom = false">
			<view class="cu-dialog" @tap.stop="">
						...
						<view class="item-list">
							...
							<view class="item-list-center">
								<view class="number-change">
									<button class="operate cu-btn" @click="changeTime('occur_time', -60)">
										-1小时
									</button>
									<button class="operate cu-btn margin-right" @click="changeTime('occur_time', -10)">
										-10分钟
									</button>
									<button class="operate cu-btn" @click="changeTime('occur_time', 10)">
										+10分钟
									</button>
									<button class="operate cu-btn" @click="changeTime('occur_time', 60)">
										+1小时
              ...
              currentSymptom &&
              currentSymptom.node_type === '症状' &&
              showSymptomDateSelector
            ">
						<textarea v-model="symptoms_remark" placeholder="输入症状说明" />
					</view>
					<view class="button-box">
						<button type="primary" class="cu-btn next-btn bg-cyan round" v-if="
                currentSymptom &&
                currentSymptom.node_type === '症状' &&
                !showSymptomDateSelector
              " @click="symptomNext('next')">
						...
						<button class="cu-btn next-btn bg-gray round" v-if="
                currentSymptom &&
                currentSymptom.node_type === '症状' &&
                showSymptomDateSelector
              " @click="symptomNext('pre')">
							上一步
						</button>
						<button type="primary" class="cu-btn next-btn bg-cyan round" v-if="
                currentSymptom &&
		...
		<view class="cu-modal bottom-modal" :class="{ show: showTypePopup }">
			<view class="cu-dialog" @tap.stop="">
				<view class="switch-type-box">
					<view class="type-item cu-btn round" :class="{ 'bg-blue': item.name === currentType }"
						v-for="item in typeList" :key="item.name" @click="currentType = item.name">
						{{ item.name }}
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
				...
				if (this.vuex_userInfo.weight && this.vuex_userInfo.height) {
					if (Array.isArray(this.historyRecord) && this.historyRecord.length > 0) {
						if (this.historyRecord[0].weight) {
							return Number(((this.historyRecord[0].weight * 10000) / this.vuex_userInfo.height ** 2)
								.toFixed(1))
						}
					} else {
		...
		methods: {
			...
			buildBPOption(data) {
				...
				let option = {
					...
					legend: {
						...
						top: '5%',
						// icon: 'roundRect',
						icon: 'path://M1635.315872 398.277883a510.609754 510.609754 0 0 0-996.200768 0H0v227.443097h639.115104a510.609754 510.609754 0 0 0 996.200768 0H2274.430976v-227.443097zM1137.215488 910.024852a398.025421 398.025421 0 1 1 398.025421-398.025421A398.025421 398.025421 0 0 1 1137.215488 910.024852z',
						itemWidth: 20,
						itemHeight: 10,
						itemGap: 10,
						data: [{
			...
			updateItem(e) {
				uni.navigateTo({
					url: '/otherPages/otherIndicator/otherIndicator?type=' + this.pageType +
						'&submitType=update&formId=' + e.id
				})
			},
			clickTag(e) {
				...
				if (e !== 'food' && e !== 'sport') {
					if (e === 'pressure' || e === 'bp') {
						...
						if (this.historyRecord.length > 0) {
							...
							url = `/otherPages/otherIndicator/otherIndicator?type=${e}&systolic_pressure=${data[ 'systolic_pressure' ]}&diastolic_pressure=${data[ 'diastolic_pressure' ]}&heart_rate=${data[ 'heart_rate' ]
              }`
						}
					}
					if (url) {
			...
			showCanvas(type) {
				...
				switch (type) {
					...
					case 'sleep':
						...
						this.getChartData('sleep').then(_ => {}) // 血压
			...
			async getChartData(type) {
				...
				if (res.data.state === 'SUCCESS' && Array.isArray(res.data.data) && res.data.data.length > 0) {
					...
					if (type === 'weight') {
						...
						this.weightChartData.categories = res.data.data.map(item => dayjs(item.create_time).format(
							'MM-DD'))
						...
						this.BPChartData.categories = res.data.data.map(item => dayjs(item.create_time).format(
							'MM-DD'))
					...
					if (type === 'weight' && this.vuex_userInfo && this.vuex_userInfo.weight) {
						this.historyRecord = [{
							create_time: this.vuex_userInfo.modify_time ? this.vuex_userInfo.modify_time : this
								.vuex_userInfo.create_time,
					...
					result.forEach(item => {
						self.energyListWrap.forEach(energy => {
							energy.matterList.forEach(mat => {
								if (item.nutrient === mat.name || item.nutrient.indexOf(mat
										.name) !== -1) {
									...
									if (mat.name === '蛋白') {
										mat.EAR = item.val_rni ? item.val_rni * self.userInfo
											.weight : item.val_ear ? item.val_ear * self
											.userInfo.weight : mat.EAR * self.userInfo.weight
										mat.UL = 0
									}
								} else {
									if (mat.name === '脂肪') {
										mat.EAR = Number((self.userInfo.weight * 50 * 0.2) / 9)
											.toFixed(2)
										mat.UL = 0
									}
									if (mat.name === '碳水') {
			...
			async getDietRecord(chooseDate = null) {
				...
				energyList.forEach(item => {
					item.matterList.forEach(mat => {
						...
						if (mat.center_width * 3 < mat.left_width || mat.center_width * 3 < mat
							.right_width) {
