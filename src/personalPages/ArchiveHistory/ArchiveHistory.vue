<template>
  <view class="history-wrap">
    ...
    <view class="button-box" v-if="isAllPages">
      <button
        class="buttons"
        :class="{ active: currentChart === item.chartID }"
        size="mini"
    ...
    <view class="history-box" v-if="pageType">
      <view class="symptom-page" v-if="pageType === 'symptom'">
        ...
        <button
          class="cu-btn bg-cyan symptom-add-button shadow-blur"
          v-if="pageType === 'symptom'"
      ...
      <view class="history-content">
        ...
        >
          <!-- 体重、血压、睡眠 -->
          <view class="record-title" v-if="pageType !== 'weight'"
            >最新数据</view
          >
          ...
          <view class="bmi-box" v-if="pageType === 'weight'">
            <view class="bmi-bar-box" v-if="bmi">
              <view class="last-data">
                <view
                  class="text-gray create-time"
                  v-if="isArray(historyRecord) && historyRecord.length > 0"
                  >{{ historyRecord[0].create_time }}</view
                >
          ...
          <view
            class="data-chart"
            v-if="pageType === 'weight' || pageType === 'bp'"
          >
            <uniEcCharts
              class="uni-ec-charts"
              id="uni-ec-charts"
              :ec="weightChartOption"
              v-if="pageType === 'weight'"
            ></uniEcCharts>
            <uniEcCharts
              class="uni-ec-charts"
              id="uni-ec-charts"
          ...
          <view class="history-record">
            ...
            <u-empty
              mode="history"
              v-if="historyRecord && historyRecord.length === 0"
            ></u-empty>
            <view
              class="list-box cu-list"
              v-if="historyRecord && historyRecord.length > 0"
            >
              <view
              ...
              >
                <image
                  src="../static/icon/xueya.png"
                  mode=""
                  class="icon"
                  v-if="pageType === 'bp'"
                ></image>
                <image
                  src="../static/icon/sleep.png"
                  mode=""
                  class="icon"
                  v-if="pageType === 'sleep'"
                ></image>
                <image
                  src="../static/icon/tizhong.png"
                  mode=""
                  class="icon"
                  v-if="pageType === 'weight'"
                ></image>
                ...
                <view class="move">
                  <view class="bg-red" @click.stop="deleteItem(item)"
                    >删除</view
                  >
                </view>
              </view>
    ...
    <view
      ...
      :class="{ show: showSymptom }"
      @click="showSymptom = false"
    >
      <view class="cu-dialog" @tap.stop="">
        <view
          class="symptom-selector"
          :class="{ 'low-height': showSymptomDateSelector }"
        >
          <cascader-selector
              ...
              <view class="item-list-center">
                <view class="number-change">
                  <button
                    ...
                    @click="changeTime('occur_time', -60)"
                  >
                    -1小时
                  </button>
                  <button
                    class="operate cu-btn margin-right"
                    @click="changeTime('occur_time', -10)"
                  >
                    -10分钟
                  </button>
                  <button
          ...
          <view class="button-box">
            ...
            <button
              class="cu-btn next-btn bg-gray round"
              v-if="
                currentSymptom &&
                currentSymptom.node_type === '症状' &&
                showSymptomDateSelector
    ...
    <view class="cu-modal bottom-modal" :class="{ show: showTypePopup }">
      <view class="cu-dialog" @tap.stop="">
        <view class="switch-type-box">
          <view
            class="type-item cu-btn round"
            :class="{ 'bg-blue': item.name === currentType }"
            v-for="item in typeList"
            :key="item.name"
            @click="currentType = item.name"
          >
            {{ item.name }}
          </view>
        </view>
