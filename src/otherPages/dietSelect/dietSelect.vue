<template>
  <view class="contents">
    ...
    <view class="handler-action" v-if="pageType !== 'sport'">
      <text
        class="margin-right"
        @click="showModal('sort')"
        :class="{ 'text-cyan text-bold': childChooseArr.length > 0 }"
      >
        <text>排序</text>
        <text class="cuIcon-order"></text>
        <text v-if="childChooseArr.length === 1" style="font-size: 12px"
          >({{ childChooseArr[0].title }})</text
        >
      </text>
      <text
        @click="showModal('filter')"
        :class="{
          'text-cyan text-bold': filterVal && filterVal.indexOf('全部') === -1,
        }"
      >
        <text>筛选</text>
        <text class="cuIcon-filter"></text>
        <text
          v-if="filterVal && filterVal.indexOf('全部') === -1"
          ...
          <view class="smallbox">
            <view class="smallbox-img">
              <image
                ...
                height="100%"
                v-if="!food.imgurl"
                src="/otherPages/static/img/none.png"
              ></image>
              <image
                width="100%"
                height="100%"
            ...
            <view class="textbox">
              ...
              <view v-if="lockEledata" class="lock-ele">
                ...
                <text class="second-lock">{{
                  lockEledata.units ? lockEledata.units.split("/")[0] : ""
                }}</text>
                )
              </view>
              <view class="food-utis">
                <text class="number" style="margin-right: 5px"
                  >{{ food.unit_energy }}千卡</text
                >
    ...
    <view
      ...
      :class="{ show: showBottomModal }"
    >
      <view
        class="cu-dialog"
        :class="pageType === 'sport' ? 'sport-dialog' : ''"
      >
        <view class="cu-bar bg-white">
          <view class="text-cyan button" @tap="showBottomModal = false"
            >取消</view
          >
          <view class="date-time">
            <view @click="openCalendar" class="date-time-rq">{{
              nowDate
            }}</view>
            /
            <view @click="openTime" class="date-time-rq">{{
              currTime ? currTime : nowDateTime
            }}</view>
          </view>
          <view @click="confirms" class="text-cyan button confir-btn"
            >确定</view
          >
        </view>
            ...
            <view class="list-item">
              ...
              <view class="info">
                ...
                <text class="infomation">
                  <text class="text-red calorie">{{
                    currFood.unit_energy
                  }}</text>
    ...
    <view class="cu-modal car-model" :class="{ show: showCarModal }">
      <view class="cu-dialog">
        ...
        <view class="padding-xl foods-cont">
          <view
            class="boxfood_car"
            v-for="(food, idx) in chooseFoodArr"
            :key="idx"
          >
            <view class="smallbox_car">
              ...
              <view class="textbox_car">
                ...
                <view class="food-utis_car">
                  <text class="number_car" style="margin-right: 5px"
                    >{{ food.energy }}千卡</text
                  >
                  <text class="utis_car"
                    >/
                    {{
                      food.unit_weight_g
                        ? food.amount * food.unit_weight_g
            ...
            <view class="relian">
              <text v-if="food.heatNum"
                >{{ food.heatNum && food.heatNum.toFixed(1) }}千卡</text
              >
              <view @click="delItem(food)" style="margin-left: 20rpx" class="">
                <uni-text class="lg text-gray cuIcon-delete"
                  ><span></span
                ></uni-text>
              </view>
            </view>
        ...
        <view class="button-box"
          ><button class="cu-btn bg-blue" @click="back">完成</button></view
        >
      </view>
    </view>
    <view
    ...
    />
    <jumpBall
      :backgroundImage="currFood.imgurl"
      :start.sync="num"
    ...
    <view class="public-button-box add-button">
      <view @click="openCar" class="lg text-gray cuIcon-cart add-button-wrap">
        <text v-show="chooseFoodArr.length > 0" class="add-button-num">{{
          chooseFoodArr.length
        }}</text>
      </view>
    </view>
    <view
      class="cu-modal drawer-modal justify-end"
      :class="{ show: modalName === 'filter' }"
      @click="hideModal"
    >
          ...
          <view class="filter-item-box">
            ...
            <view class="child-data">
              <cascader-selector
                ...
                insert
                hideButton
                @clickTag="changeFilter($event, '食材')"
                :srvInfo="dietClassifySrv"
              ></cascader-selector>
            </view>
          </view>
        </view>
        <view class="button-box"
          ><button type="primary" class="cu-btn bg-blue" @click="hideModal">
            确定
          </button></view
        >
      </view>
        ...
        <view class="filter-box">
          <view class="filtrate-wrap">
            <view class="filtrate-item-wrap">
              <view
                v-for="(item, index) in menuAgList"
                :key="index"
                class="filtrate-item filter-item-box"
              >
                <view class="filtrate-item-left label">{{
                  item.classify_name
                }}</view>
                <view class="" style="display: flex; flex-flow: wrap">
                  <view
                    @click="chooseMenu(item, cate)"
                    v-for="(cate, i) in item.children"
                    :key="i"
                    :class="cate.choose ? 'cate-active' : ''"
            ...
            <view v-if="childChooseArr.length > 0" class="filtrate-choose">
              ...
              <view
                @click="tagClick(item)"
                v-for="(item, index) in childChooseArr"
                :key="index"
                class="filtrate-choose-item"
              >
                <text
                  class="cu-tag"
                  :text="item.title"
  ...
  methods: {
    ...
    tofeedback () {
      ...
      if (!this.isShowMyList) {
        ...
        uni.navigateTo({
          url: '/publicPages/form/form?serviceName=srvhealth_mixed_food_nutrition_contents_add&type=add&addType=onwer&fieldsCond=' + decodeURIComponent(JSON.stringify(cond))
        });
      }
    },
    countDietNum (num) {
