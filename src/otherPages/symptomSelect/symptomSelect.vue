<template>
  <view class="symptoomSelectWrap">
    <view class="symptopm-tops">
      ...
      <Thetable
        :list="chooseArr"
        @delAlltabAllansw="delAlltabAllansw"
        ref="tabbox"
        @delHtml="deltab"
      ></Thetable>
      <view class="box">
        <view class="cu-bar search bg-white">
          ...
          <view class="action">
            <button
              v-if="isSearch"
              @click.stop="searchEnd"
              class="cu-btn bg-orange shadow-blur round"
            >
              取消
            </button>
          </view>
    ...
    <view class="symptom-bot-wrap">
      <view
        class="symptom-bot-wrap-main"
        v-if="
          symptomList.length && symptomList[0].children.length > 0 && !isSearch
        "
      >
        <view
          v-if="symptomList[0].is_show && items.is_leaf === '否'"
          :key="index"
          v-for="(items, index) in symptomList[0].children"
          class="wrapCont"
        >
          <view
            v-if="items.is_leaf === '否' && items.children.length > 0"
            class="wrapCont_row"
          >
            ...
            <view class="wrapCont-main-no">
              <view
                ...
                :key="nos"
                v-if="
                  !Array.isArray(noillnes.children) ||
                  noillnes.children.length <= 0
                "
                class="wrapCont_row_item_wrap-b"
              >
                <view
                  @click="chooseItem(noillnes)"
                  :class="noillnes.is_checked ? 'actived' : ''"
            ...
            <view class="wrapCont-main">
              <view
                v-if="
                  Array.isArray(illnes.children) && illnes.children.length > 0
                "
      ...
      <view v-else class="wrapCont">
        <view class="box" v-show="serseData.length > 0">
          <view
            ...
            v-for="(item, index) in serseData"
            :key="index"
            class="wrapCont_row_item"
            :class="item.is_checked ? 'actived' : ''"
          >
            {{ item[query.key] }}
          </view>
        </view>
        <view class="box normalstyle" v-show="serseData.length == 0"
          >没有找到相关内容</view
        >
      </view>
      <view class="boxbtn"
        ><view class="btns" @click="lookobj">完成</view></view
      >
    </view>
    ...
    <view class="cu-modal bottom-modal" :class="{ show: menuIsShow }">
      <view class="cu-dialog">
        <view
          class="action pregnant-main-top-item-poup-top"
          @tap="menuIsShow = false"
        >
