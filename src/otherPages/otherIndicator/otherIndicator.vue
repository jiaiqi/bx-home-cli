<template>
  <view class="other-wrap">
    <view class="other-top">
      ...
      <view v-if="type && type === 'weight'" class="item-wrap">
        ...
        <view class="item-list">
          ...
          <slider-number
            v-model="inputVal.body_fat_rate"
            :max="50"
            :min="0"
            :step="0.1"
          ></slider-number>
        ...
        <view class="item-list">
          <view class="item-list-top">
            ...
            <view class="value" v-if="inputVal.measure_time">
              <picker
                mode="date"
                :value="inputVal.measure_time.split(' ')[0]"
                @change="DateChange($event, 'measure_time')"
              >
                <view class="picker">{{
                  inputVal.measure_time.split(" ")[0]
                }}</view>
              </picker>
              <view class="margin-right"></view>
              <picker
                mode="time"
                :value="inputVal.measure_time.split(' ')[1]"
                @change="TimeChange($event, 'measure_time')"
              >
                <view class="picker">{{
                  inputVal.measure_time.split(" ")[1]
                }}</view>
              </picker>
            </view>
          </view>
          <view class="number-change">
            <button
              ...
              @click="changeTime('measure_time', -60)"
            >
              -1小时
            </button>
            <button
              class="operate cu-btn margin-right"
              @click="changeTime('measure_time', -10)"
            >
              -10分钟
            </button>
            <button
      ...
      <view v-else-if="type && type === 'sleep'" class="item-wrap">
        <view class="item-list">
          <view class="item-list-top">
            ...
            <view class="value" v-if="inputVal.retire_time">
              <picker
                mode="date"
                :value="inputVal.retire_time.split(' ')[0]"
                @change="DateChange($event, 'retire_time')"
              >
                <view class="picker">{{
                  inputVal.retire_time.split(" ")[0]
                }}</view>
              </picker>
              <view class="margin-right"></view>
              <picker
                mode="time"
                :value="inputVal.retire_time.split(' ')[1]"
                @change="TimeChange($event, 'retire_time')"
              >
                <view class="picker">{{
                  inputVal.retire_time.split(" ")[1]
                }}</view>
              </picker>
            </view>
          </view>
          <view class="number-change">
            <button
              class="operate cu-btn"
              @click="changeTime('retire_time', -60)"
            >
              -1小时
            </button>
            <button
              class="operate cu-btn margin-right"
              @click="changeTime('retire_time', -10)"
            >
              -10分钟
            </button>
            <button
        ...
        <view class="item-list">
          <view class="item-list-top">
            ...
            <view class="value" v-if="inputVal.getup_time">
              <picker
                mode="date"
                :value="inputVal.getup_time.split(' ')[0]"
                start="2021-01-01"
                end="2030-09-01"
                @change="DateChange($event, 'getup_time')"
              >
                <view class="picker">{{
                  inputVal.getup_time.split(" ")[0]
                }}</view>
              </picker>
              <view class="margin-right"></view>
              <picker
                mode="time"
                :value="inputVal.getup_time.split(' ')[1]"
                start="07:00"
                end="23:00"
                @change="TimeChange($event, 'getup_time')"
              >
                <view class="picker">{{
                  inputVal.getup_time.split(" ")[1]
                }}</view>
              </picker>
            </view>
          </view>
          <view class="number-change">
            <button
              class="operate cu-btn"
              @click="changeTime('getup_time', -60)"
            >
              -1小时
            </button>
            <button
              class="operate cu-btn margin-right"
              @click="changeTime('getup_time', -10)"
            >
              -10分钟
            </button>
            <button
        ...
        <view class="item-list">
          ...
          <bx-checkbox-group v-model="inputVal.sleep_event" mode="button">
            <bx-checkbox
              ...
              class="checkbox"
              color="#2979ff"
              v-for="(item, i) in sleepEventOption"
              :key="item.value"
              :name="item.value"
            >
              {{ item.label }}
        ...
        <view class="item-list">
          ...
          <bx-checkbox-group v-model="inputVal.wake_up_status" mode="button">
            <bx-checkbox
              ...
              class="checkbox"
              color="#2979ff"
              v-for="(item, i) in wakeUpStatusOption"
              :key="item.value"
              :name="item.value"
            >
              {{ item.label }}
        ...
        <view class="item-list">
          ...
          <slider-number
            v-model="inputVal.urinate_times"
            :max="10"
            :min="0"
          ></slider-number>
      ...
      <view
        v-else-if="type && (type === 'pressure' || type === 'bp')"
        class="item-wrap"
      >
        <!-- 血压 -->
        <view class="item-list">
        ...
        <view class="item-list">
          ...
          <slider-number
            v-model="inputVal.systolic_pressure"
            :max="200"
            :min="50"
          ></slider-number>
        </view>
        <view class="item-list">
          <view class="item-list-top">
            ...
            <text class="margin-left-xs">{{
              inputVal.diastolic_pressure
            }}</text>
          </view>
          <slider-number
            v-model="inputVal.diastolic_pressure"
            :max="120"
            :min="50"
          ></slider-number>
        </view>
      ...
      <view v-else-if="type && type === 'glucose'" class="item-wrap">
        ...
        <view class="item-list" @click="showSelect = true">
          ...
          <bx-radio-group v-model="inputVal.glucose_time" mode="button">
            <bx-radio
              class="radio"
              color="#2979ff"
              v-for="item in glucose_time_option"
              :key="item.value"
              :name="item.value"
              >{{ item.value }}</bx-radio
            >
        ...
        <view class="item-list">
          ...
          <slider-number
            v-model="inputVal.blood_glucose_val"
            :max="28"
            :min="0.5"
            :step="0.1"
          ></slider-number>
    ...
    <view class="other-bot">
      <button
        type="primary"
        class="cu-btn btn bg-cyan shadow-blur"
      ...
      <button class="cu-btn bg-grey btn" @click="back" v-if="isSubmit">
        返回
      </button>
      <button
        class="cu-btn sm round bg-red margin-left-xs"
        @click="deleteItem"
  ...
  computed: {
    ...
    canSave () {
      ...
      switch (this.type) {
        ...
        case 'oxygen':
          result =
            this.inputVal.start_time && this.inputVal.end_time && this.inputVal.oxygen_saturation_max && this.inputVal.oxygen_saturation_min &&
                ...
                uni.showModal({
                  ...
                  success (e) {
                    if (e.confirm) {
                      // uni.navigateBack();
                      uni.redirectTo({
                        url: '/archivesPages/archives-history/archives-history?pageType=' + self.type
                      })
                    }
                  }
                });
      ...
      switch (this.type) {
        ...
        case 'sleep':
          ...
          let obj = {
            ...
            sleepy_daytime: this.inputVal.sleepy_daytime.toString(),
