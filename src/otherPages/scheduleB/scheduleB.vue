...
<script>
	...
	export default {
		...
		computed: {
			rowHeadCustomData() {
				let data = null
				if (this.scheduleConfig?.row_head_custom_data) {
					try {
						data = JSON.parse(this.scheduleConfig.row_head_custom_data)
					} catch (e) {
			...
			buildTableData() {
				...
				if (config.row_head_custom_data) {
					try {
						let row_head_custom_data = JSON.parse(config.row_head_custom_data)
							...
							scheduleList.data.forEach(schedule => {
								rowHeadList.forEach((row, rowIndex) => {
									...
									colHeadList.forEach((col, colIndex) => {
										...
										if (!schRowVal) {}
										if (min && max && schRowVal) {
											schRowVal = parseInt(schRowVal)
										}
										if (rowVal === schRowVal || (min && max &&
												schRowVal >= min && schRowVal < max) || (!
												schRowVal && min == 8 && max == 10)) {
											let obj = {}
											if (colVal === schColVal) {
...
<style lang="scss" scoped>
	...
	.course {
		::v-deep.ws-table .table-content .content .left .row {
