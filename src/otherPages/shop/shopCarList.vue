<template>
  <view class="car-list-wrap">
        ...
        <view @click="choseItem(item)" class="item-left-wrap">
          ...
          <view class="item-right">
            <view class="item-right-l"
              ><image :src="getImagePath(item.image)" mode="aspectFill"></image
            ></view>
    ...
    <view v-else class="none-data"
      ><text>还没有添加任何商品，快去添加吧</text></view
    >
    <view class="car-bot">
      ...
      <view
        :class="isShowDel ? '' : 'none_shop_bottom_rig'"
        class="shop_bottom_rig"
      >
        <text @click="del">删除所选</text>
        <text @click="payOrder"
          >结算({{ chooseCarData.length > 0 ? chooseCarData.length : 0 }})</text
        >
      </view>
    </view>
  </view>
