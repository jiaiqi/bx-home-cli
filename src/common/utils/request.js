export default function({
	...
	return {
		...
		uploadFile(url, data, header, ...args) {
			return this.file('uploadFile', url, data || {}, { ...header,
				...headers
			}, ...args)
		},
		downloadFile(url, data, header, ...args) {
			return this.file('downloadFile', url, data || {}, { ...header,
				...headers
			}, ...args)
		},
		...
		file(method, url, data, header, reqIntercept, resIntercept) {
			...
			return new Proxy(new Promise((resolve, reject) => { // 返回经过 Proxy 后的 Promise 对象使其可以监听到是否调用 abort 和 onProgressUpdate 方法
				this.interceptors.request.intercept({
					header: header || {},
					body: data.formData || {}
				}, method, url, data, reqIntercept).then(async ({
					header,
					body,
					cancel
				}) => { // 等待请求拦截器里的方法执行完
					...
					requestTask = uni[method]({
						...
						success: async res => {
							clearTimeout(timer);
							!statusCode.includes(res.statusCode) ? await this.onerror(method, url, data,
