...
fly.interceptors.request.use(async (request) => {
	...
	if (request.url && request.url.indexOf('srvwx_app_login_verify') == -1 && request.url.indexOf(
		'rvuser_login') == -1) {
		if (store && store.getters && store.getters.isLogin ===
			false) {
			// request.cancel = true
			// #ifdef H5
			uni.navigateTo({
